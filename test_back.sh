...
function base_new_wallet() {
	...
	pubkey_1=`./baic_cli wallet create_key| awk 'BEGIN { FS="[\"]" }  {print $2}'`
	pubkey_2=`./baic_cli wallet create_key| awk 'BEGIN { FS="[\"]" }  {print $2}'`
	
	active_pub_key=$pubkey_1	
	owner_pub_key=$pubkey_2
	
	#active_private_key= #low permission
	#owner_private_key=  #full permission

	#$cli_path/./baic_cli wallet import -n baic --private-key 5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3
	$cli_path/./baic_cli wallet import -n baic --private-key $active_private_key
...
spawn $cli_path/./baic_cli wallet private_keys
